...
const InNeed = forwardRef(function ({ visible, id }, ref) {
  ...
  const { height, mobile, tablet, tabletLarge, desktopSmall } = useContext(ScreenModeAndSizeContext)
    ...
    <section className={`${style.section} section ${visible ? 'showText' : ''}`} id={id} ref={ref}>
      <div className={style.block}>
        ...
        <div className={`${style.text} h2 textContainer`}>
          <h2 className={`h2 ${lang === 'ua' ? style.ua : ''}`}>{texts.inNeed.paragraph[lang]}</h2>
        </div>
      </div>
    </section>
  )
