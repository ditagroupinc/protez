...
export default function Header({ notMainPage, black }) {
  ...
  const navLinks = Object.keys(texts.header.navigation).map(el => texts.header.navigation[el])
  const toggleHeader = () => {
    setHeaderIsOpened(!headerIsOpened)
  }

  const blackClassName = () => {
    if (notMainPage || black || (width <= 750 && headerIsOpened)) return style.black
    return ''
  }
  ...
  return (
    <>
      <header
        className={`${style.header} ${notMainPage ? style.notMainPage : ''} ${blackClassName()}`}
        id="header"
      >
        {notMainPage ? (
          <Link href={'/'}>
