...
Блок 5 ──┤                          │
         └──► Блок 6 ──► Блок 7 ────┘
                  │
                  └──► Блок 9 (опційний, після 6)
```
Блоки 1–5 повністю незалежні; виконуються в будь-якому порядку. Блок 6 робиться після 3 (структура секцій) і бажано після 5 (стилі). Блок 7 — одразу після 6. Блок 8 — у самому кінці. Блок 9 — опційна оптимізація після Блоку 6 (до або після 7/8).
...
9. **PR #9 — Блок 9:** Звуження клієнтських компонентів до leaf-островів
...
## Блок 9 — Звуження клієнтських компонентів до leaf-островів

> **Статус:** чернетка. Деталі будуть прописані пізніше — тут зафіксована лише загальна ідея, щоб не загубилася.

### Контекст блоку
Після Блоку 6 усі сторінки рендеряться як SSG (`●` у `next build`), але **обсяг гідрації** великий: майже всі секції позначені `'use client'`, бо в кожній є щонайменше один клієнтський хук (`react-slick`, `framer-motion`/`useInView`, `useState` для слайдерів, `react-countup`, `useScreenModeAndSize`). Як наслідок, серверний RSC-дерево обривається на рівні `_root`-композерів, а `First Load JS` на головній — ~180 KB.

Мета Блоку 9 — **просунути межу клієнт/сервер усередину**: композери та більшість презентаційних секцій робляться RSC (`getTranslations` замість `useTranslations`), а власне інтерактивна логіка виноситься у вузькі острівці в `src/islands/*`. SSG лишається, але JS-бандл після гідрації істотно зменшується.

### Загальна ідея
- **Композери у RSC:** `src/sections/home/_root`, `src/sections/academy/_root`, `app/[locale]/academy/about/AcademyAboutClient` → перевести на серверну рендер-модель. `lazy()` замінити на звичайний `import` або `next/dynamic` з `ssr: true` де можливо.
- **Виокремити справжні острівці у `src/islands/`:**
  - `LangSwitcher` (зараз сидить усередині Header)
  - `BurgerMenu` (мобільне меню Header / AcademyHeader)
  - `SlickCarousel` універсальна обгортка над `react-slick` із локальним імпортом slick-CSS
  - `ScrollReveal` — обгортка над `react-intersection-observer` для анімацій появи
  - `CountUp` — обгортка над `react-countup` для статистик
  - `MailingListForm`, `ContactForm` — клієнтські форми
  - `LazyMotion` / `VideoPlayer` — там, де реально треба
- **Презентаційні секції стають RSC.** Туди, де секції лише читають переклади і не мають жодних клієнтських хуків, прибрати `'use client'`, переключити на `getTranslations`.
- **Header / AcademyHeader розпиляти:** shell-RSC, що рендерить статичну розмітку й слоти, + три-чотири острівці всередині (LangSwitcher, BurgerMenu, BackToTopButton).
- **Інкрементально, по сторінках.** Кожна сторінка — окремий під-PR, щоб легко відкочувати, виміряти `First Load JS` до/після і не змішувати з фінальним прибиранням.

### Можливе розбиття на під-PR'и (фіналізувати у детальному плані)
1. **9a — Home.** `_root` композер + 14 home-секцій. Найбільший виграш, бо головна — найважча.
2. **9b — Academy + Academy About.** Композери + секції з gallery/slider'ами.
3. **9c — Donate / Partners / Stories / Thank-you.** Простіші сторінки; зокрема `terms-conditions` уже RSC, бо переписана у Блоці 6.

### Метрики, які треба порівнювати «до/після» у кожному під-PR
- `First Load JS` для маршруту з `next build`.
- Кількість `'use client'` директив у `src/sections/` і `src/components/` (`grep -rl "use client" src/sections src/components | wc -l`).
- Lighthouse / Vercel Speed Insights (LCP, TBT) на превʼю-деплої.

### Чого Блок 9 **не** робить
- Не міняє SSG-стратегію — SSG вже працює після Блоку 6.
- Не зачіпає тексти/переклади — namespace'и messages лишаються ті ж.
- Не зачіпає URL-структуру, redirect'и, sitemap.
- Не оптимізує зображення/шрифти/стилі — це Блоки 1, 4, 5.

### Перевірка (рамка, фінал — у детальному плані)
- **ESLint:** `npm run lint`.
- **TypeScript:** `npx tsc --noEmit`.
- **Build:** `npm run build` — усі маршрути лишаються `●`, `First Load JS` менший, ніж до Блоку 9.
- **Playwright MCP:** ті самі smoke-сценарії, що в Блоках 6 і 8 — переконатися, що візуально і функціонально нічого не зламано (інтерактивні слайдери, форми, перемикач мов, burger-меню).
